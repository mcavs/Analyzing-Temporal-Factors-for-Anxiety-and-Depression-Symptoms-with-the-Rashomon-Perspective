...
for (i in 1:N) {
  ...
  models[[i]]     <- randomForest(factor(GAD2_score_cat) ~ age + education + sex + weekday_weekend + hour_numeric,
                                  data = ndf[[i]])
  explainers[[i]] <- DALEX::explain(models[[i]],
                                    data = ndf[[i]][, c(8, 9, 10, 11, 15, 16)],
                                    y = ndf[[i]]$GAD2_score_cat,
                                    verbose = FALSE)
  profiles[[i]]   <- model_profile(explainers[[i]], variable_splits = list(age = 18:80), N = NULL)
  profiles_w[[i]] <- model_profile(explainers[[i]], groups = "sex",
                                   variable_splits = list(age = 18:80), N = NULL)
...
saveRDS(all_profiles_w, "profiles_age_vs_sex_for_GAD2.rds")
