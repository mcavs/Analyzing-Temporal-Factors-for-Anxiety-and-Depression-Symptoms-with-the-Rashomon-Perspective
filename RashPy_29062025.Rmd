...
model <- randomForest(PHQ2_score_cat ~ age + education + sex + weekday_weekend + hour_numeric, 
                      data = datadf)
...
```{r echo=TRUE, message=FALSE, warning=FALSE}
model <- randomForest(GAD2_score_cat ~ age + education + sex + weekday_weekend + hour_numeric, 
                      data = datadf)
modelex <- explain(model, data = datadf[,c(8, 9, 10, 11, 15, 16)], 
